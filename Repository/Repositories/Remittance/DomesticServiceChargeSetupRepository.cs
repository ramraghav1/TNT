...
{
    public class DomesticServiceChargeSetupRepository : IDomesticServiceChargeSetupRepository
    {
        private readonly IDbConnection _dbConnection;

        public DomesticServiceChargeSetupRepository(IDbConnection dbConnection)
        {
        ...
        public SetupDetailResponse Create(CreateSetupRequest request)
        {
            ...
            {
                string sqlSetup = @"
                    INSERT INTO domestic_service_charge_setups
                    (from_category_id, to_category_id, payment_type_id, agent_id,
                     charge_mode, is_active, created_at)
                    VALUES
                    (@FromCategoryId, @ToCategoryId, @PaymentTypeId, @AgentId,
                     @ChargeMode, true, NOW())
                    RETURNING id;";
        ...
        public List<SetupListItem> GetAll()
        {
            string sql = @"
                SELECT
                    s.id,
                    s.from_category_id,
                    fc.display_name AS from_category_name,
                    s.to_category_id,
                    tc.display_name AS to_category_name,
                    s.payment_type_id,
                    ...
                    s.created_at,
                    s.updated_at
                FROM domestic_service_charge_setups s
                JOIN configurations fc ON fc.id = s.from_category_id
                JOIN configurations tc ON tc.id = s.to_category_id
        ...
        public SetupDetailResponse? Update(long id, UpdateSetupRequest request)
        {
            ...
            {
                string sql = @"
                    UPDATE domestic_service_charge_setups
                ...
                {
                    _dbConnection.Execute("DELETE FROM domestic_service_charge_slabs WHERE setup_id = @Id;", new { Id = id }, transaction);
        ...
        public bool Delete(long id)
        {
            ...
            string sql = "DELETE FROM domestic_service_charge_setups WHERE id = @Id;";
        ...
        public CalculateChargeResponse? CalculateCharge(CalculateChargeRequest request)
        {
            ...
            string sqlSetup = @"
                ...
                FROM domestic_service_charge_setups
                WHERE from_category_id = @FromCategoryId
                  AND to_category_id = @ToCategoryId
